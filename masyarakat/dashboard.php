<p style="font-size: 40px; font-family: sans-serif;">Aplikasi Pengaduan Masyarakat</p>
<?php
session_start();
		...
		<td>
			...
			<table border="3" class="responsive-table striped highlight">
				...
				$pengaduan = mysqli_query(
					$koneksi,
					"SELECT * FROM tanggapan RIGHT JOIN pengaduan ON tanggapan.id_pengaduan=pengaduan.id_pengaduan
				INNER JOIN masyarakat ON pengaduan.nik=masyarakat.nik
				LEFT JOIN petugas ON tanggapan.id_petugas=petugas.id_petugas
				WHERE pengaduan.nik=$id_nik ORDER BY pengaduan.id_pengaduan DESC"
				);
...
<p>Selamat datang di Aplikasi Pengaduan Masyarakat, Silahkan beri laporan di atas.<br>
	Petugas merespon 1x24jam khusus kecamatan Cilodong Depok,<br>
	secepatnya akan kami segera proses kasus laporan yang anda berikan.</p>